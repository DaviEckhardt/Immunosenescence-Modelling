...
outdir = os.path.join(os.path.dirname(__file__), "modelo_acoplado_figures_dif_d")
os.makedirs(outdir, exist_ok=True)

base_params = {
    'πv': 6.80e-1, 'cv1': 2.63e0, 'cv2': 6e-1, 'kv1': 4.82e-5, 'kv2': 7.48e-7,
    'αap': 2.5, 'βap': 5.5e-1, 'cap1': 8e-1, 'cap2': 4e1, 'δapm': 5.38e-1,
    'αth': 2.17e-4, 'βth': 1e-7, 'πth': 1e-8, 'δth': 2.2e-1,
    'αtk': 2.17e-4, 'βtk': 1e-5, 'πtk': 1e-8, 'δtk': 3e-4,
    'αb': 6.0e0, 'πb1': 4.83e-6, 'πb2': 1.27e-8,
    'βps': 6.72e-4, 'βpl': 5.61e-6, 'βbm': 1e-6,
    'δps': 2.0e0, 'δpl': 2.4e-4, 'γbm': 9.75e-4,
    'πbm1': 1e-5, 'πbm2': 2.5e3, 'πps': 2e-3, 'πpl': 6.8e-4, 'δa': 4e-2,
    'Ap0': 1e6, 'Thn0': 1e6, 'Tkn0': 5e5, 'B0': 2.5e5,
...
y0 = [
    724.0, 1e6, 0, 1e6, 0, 5e5, 0, 2.5e5, 0, 0, 0, 150, 0.1, 0.0, 0.0
]
...
for dval in d_values:
    p = base_params.copy()
    p['d'] = dval
    sol = odeint(lambda y, t: coupled_model(y, t, p), y0, t)
    results[dval] = sol
...
for var_name, idx in vars_idx.items():
    plt.figure(figsize=(7, 4))
    for dval, color in zip(d_values, colors):
        sol = results[dval]
        plt.plot(t, sol[:, idx], label=f"d = {dval}", color=color)
    plt.title(var_name)
    plt.xlabel("Tempo (dias)")
    plt.ylabel("Nível relativo")
    plt.legend()
    plt.grid(True)
    fname = f"{var_name.split(' ')[0]}_comparativo_d.png".replace("/", "_")
    plt.tight_layout()
